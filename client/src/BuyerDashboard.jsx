  ...
  return (
    <main className="buyer-dashboard">
      <aside className="buyer-sidebar">
        ...
        <nav className="buyer-nav" aria-label="Buyer dashboard navigation">{navItems.map(({ id, label, icon: Icon }) => <button className={activeSection === id ? 'buyer-nav-item active' : 'buyer-nav-item'} key={id} onClick={() => selectSection(id)}><Icon size={18} /><span>{label}</span>{id === 'messages' && messages.filter((message) => !message.is_read).length > 0 && <b>{messages.filter((message) => !message.is_read).length}</b>}</button>)}</nav>
      ...
      <section className="buyer-main">
        ...
        {activeSection === 'orders' && <Orders cart={cart} cartTotal={cartTotal} orders={orders} dataLoading={dataLoading} onRemove={onRemoveFromCart} onBrowse={() => selectSection('browse')} onOpenCart={onOpenCart} onCheckout={onCheckout} />}
        {activeSection === 'messages' && <Messages messages={messages} dataLoading={dataLoading} />}
...
function Messages({ messages, dataLoading }) {
  const latest = messages[0]
  return <div className="buyer-content"><div className="buyer-section-heading"><div><p className="buyer-kicker">Stay connected</p><h2>Messages</h2></div><button className="buyer-primary compact"><MessageCircle size={16} /> New message</button></div>{dataLoading ? <div className="dashboard-empty"><MessageCircle size={32} /><h3>Loading your messages...</h3></div> : latest ? <div className="message-layout"><div className="conversation-list"><div className="conversation active"><span className="conversation-avatar">{initials(latest.participantName)}</span><span><strong>{latest.participantName}</strong><small>{latest.content}</small></span>{!latest.is_read && <b>1</b>}</div></div><div className="chat-panel"><div className="chat-head"><span className="conversation-avatar">{initials(latest.participantName)}</span><div><strong>{latest.participantName}</strong><small>Conversation</small></div></div><div className="chat-body"><div className={latest.sender_id === latest.receiver_id ? 'chat-bubble sent' : 'chat-bubble received'}>{latest.content}</div><div className="chat-time">{new Date(latest.created_at).toLocaleString()}</div></div></div></div> : <div className="dashboard-empty"><MessageCircle size={32} /><h3>No messages yet.</h3><p>Your conversations with businesses will appear here.</p></div>}</div>
}
